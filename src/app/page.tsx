'use client'
import { LocationData, CurrentWeatherData, ForecastData } from '@/interfaces/weatherinterface';
...
export default  function Home()
{
  ...
  const [curData, setCurData] = useState<CurrentWeatherData>();
  const [forCast, setForCast] = useState<ForecastData>();
  ...
  return (
    ...
    {firstLoad == true ? <>
      ...
      <div className='grid lg:grid-cols-5 grid-cols-1 gap-5 size-[90%] place-self-center align-middle my-4'>
        ...
        <h1>{locData?.[0] ? `${locData[0].name}, ${locData[0].state}` : 'Unknown location'}</h1>
